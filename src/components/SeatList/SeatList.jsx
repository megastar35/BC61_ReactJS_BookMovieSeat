...
const SeatList = () => {
  const [numberOfSeats, setNumberOfSeat] = useState([]);
          ...
          {arrSeat.map((item, index) => {
            return (
              <tr key={index} className="my-5">
                <td className="text-white">{item.hang}</td>
                {item.danhSachGhe.map((num) => {
                  return (
                    <td key={num.soGhe} className="text-center text-white ">
                      {item.hang === "" ? (
                        <span>{num.soGhe}</span>
                      ) : (
                        <SeatItem
                          soGhe={num.soGhe}
                          onSelect={() => handleSeatSelect(num)}
                          selected={selectedSeats.some(
                            (selectedSeat) => selectedSeat.soGhe === num.soGhe
                          )}
                        />
                      )}
                    </td>
